...
- **Eager, categorized environment validation at process startup** — every app (and the worker) now validates its configuration, and a set of known-dangerous production _combinations_ (default dev database credentials, memory storage, `ROOT_DOMAIN=localhost`), before serving a single request, not on first use. (v1.0.1 later scoped exactly _what_ each process validates to what it actually needs — see below.)
...
## What v1.0.1 — Production Validation & Operational Hardening adds

Also not a feature release — v1.0 built the production foundation; v1.0.1
turns four of its own self-disclosed "should normally work" gaps into
"actually run and verified." No product feature was added, no new
architecture introduced (no new queue, no Redis, no Kubernetes, no
Terraform, no database change, no new auth system, no new storage
abstraction). See this release's final report for the full detail behind
each item below.

- **Docker build validation moved from "the YAML looks right" to real evidence, plus new runtime smoke-testing.** All three images (`web`/`admin`/`worker`) were confirmed building successfully via real GitHub Actions job results (not just reading the workflow file), and `.github/workflows/ci.yml`'s `docker-build` job now also actually starts each built image on the runner's own Docker daemon: `/health/live`/`/health/ready` for web/admin against a real Postgres service container, a real `docker stop` (SIGTERM) lifecycle check for the worker (startup event logged, clean exit 0), and a non-root-user check for all three. This sandbox's own development session still cannot run `docker build`/`docker run` at all (its egress policy blocks the base-image registry) — the GitHub-hosted runner is where this is actually proven, not locally.
- **The storage smoke-test anomaly was root-caused, not worked around.** `pnpm --filter @provence360/media run smoke:storage` hung on `put` in a v1.0 manual test against `s3rver`. The real mechanism: `S3_FORCE_PATH_STYLE` defaulted to `false` unconditionally, so the AWS SDK computed virtual-hosted-style addressing (`https://<bucket>.<host>/...`) against any non-AWS `S3_ENDPOINT` — a hostname no self-hosted S3-compatible server has real DNS for — with no client-side request/connection timeout anywhere in the call path to bound the resulting failure. Fixed at the root: `S3_FORCE_PATH_STYLE` now defaults to `true` automatically whenever `S3_ENDPOINT` is set (an explicit value always still wins), and `S3ObjectStorage` now sets bounded connection/request timeouts. Reproduced with the pre-fix code (a real `getaddrinfo ENOTFOUND` against a real `s3rver` instance) and confirmed fixed with the post-fix code (a real, successful put/get/list/delete round trip) — see `packages/media/src/storage/config.test.ts`'s new regression suite.
- **`pnpm db:seed`/`pnpm db:publish-seed` now refuse an unsafe target.** `assertSeedSafeTarget()` (`packages/database/src/seed-safety.ts`) runs before either script's first write: `NODE_ENV=production` is refused unconditionally (no override), and every other target must show at least one independent positive signal (CI detected, or the database name matching this repo's `_dev`/`_test` naming convention) or it's refused too — an "ambiguous" target is treated the same as a production one. `db:migrate`/`db:setup-roles` are deliberately not gated. Verified for real against a live Postgres database: refusal happens before any connection opens, confirmed by an unchanged row count before/after.
- **Per-process environment validation, built from a real audit of each app's imports.** Through v1.0, every process (including the worker, which has no database access at all yet) parsed the full four-role `dbEnvSchema` — not just at the startup-validation layer, but inside `packages/database`'s own per-role pool getters, so narrowing only the startup check wouldn't have actually freed a process from needing variables it never uses. `loadWebEnv()`/`loadAdminEnv()`/`loadWorkerEnv()` (`packages/validation/src/env.ts`) now each validate exactly what that process's real code paths touch — web needs the resolver + app DB roles, admin adds the auth role, the worker needs nothing beyond `NODE_ENV` today — and `client-app.ts`/`client-resolver.ts`/`client-auth.ts` were fixed to match, so the fix holds at runtime, not just at boot.
- **Two real Docker runtime bugs found and fixed by the new SUJET A runtime smoke-testing** — neither would have been caught by `docker build` alone. `apps/worker/Dockerfile`'s `CMD` invoked the pnpm-generated `node_modules/.bin/tsx` shim, which pnpm's isolated linker never hoists to the workspace-root `node_modules/.bin/` this image actually copies (`tsx` is a direct dependency of `@provence360/worker` only); fixed by invoking `node --import tsx` instead, which uses Node's own module resolution from the process's CWD. Separately, `apps/web`'s and `apps/admin`'s standalone output crashed on startup with `Cannot find module '.../@swc/helpers/esm/_interop_require_default.js'`: Next's internal `require("@swc/helpers/_/_interop_require_default")` resolves to the ESM file under Node 22's `module-sync` exports condition at actual runtime, but the standalone build's file tracer (`@vercel/nft`) doesn't evaluate that condition and only traces the CJS variant — a genuine tracing gap, reproduced locally by running the real standalone `server.js` directly (Docker not required to reproduce it). Fixed with `outputFileTracingIncludes` in both `next.config.mjs` files, forcing the whole `@swc/helpers` package into every route's trace.

## What v1.0.2 — First Real Deployment adds

Also not a feature release — v1.0/v1.0.1 built and validated the
production foundation in the abstract; v1.0.2 is the first time this
codebase is actually pointed at a real deployment target. No product
feature was added, no new architecture introduced. See this release's
final report for the full detail behind each item below.

- **Railway is now a documented, concrete deployment target**, not just "any host works" — [docs/RAILWAY.md](RAILWAY.md), plus one small `railway.json` per app (`build.dockerfilePath` + `deploy.healthcheckPath`/`restartPolicyType`) making the build/health/restart contract explicit and reviewable rather than dashboard-only tribal knowledge.
- **`pnpm db:verify-roles` (new)** — a standalone script that connects as all four database roles simultaneously and asserts, with real queries, that RLS is actually enforced for `provence360_app`, that `provence360_resolver`/`provence360_auth` genuinely cannot exceed their narrow grants, and that the schema-owning role is a distinct identity from all three. Run for real against the local dev database this release (14/14 checks passed) as proof the mechanism works before applying it to a real Railway database.
- **`pnpm db:bootstrap-production` (new)** — closes the real gap `db:seed`'s production ban left open: a brand-new production database had no self-service way to create its first owner/tenant/site, since `apps/admin` has no signup flow. This script is the minimal, explicit, operator-driven answer (every value supplied via required env vars, no defaults, idempotent by tenant slug, password never logged) — see `packages/database/src/scripts/bootstrap-production.ts` and docs/RAILWAY.md, "Bootstrap the first tenant."
- **A real remote smoke test** (`apps/admin/e2e-remote/remote-smoke.spec.ts`, via `apps/admin/playwright.remote.config.ts`) drives a real headless browser against an already-deployed admin URL: health checks, login, create a page, publish, then confirms the public site actually serves the newly-published content. Safe to re-run after every redeploy. Found and fixed a real bug while being written: a site can never publish at all until it has an active home page, which is easy to miss since the Publishing page's own "why can't I publish" message is the only place that says so — see this release's final report, ANOMALIES DÉCOUVERTES.
- **Domain routing needed zero code changes for Railway's own `*.up.railway.app` domains.** Auditing `packages/domains`' resolver confirmed there is no dynamic `<slug>.ROOT_DOMAIN` computation — every reachable hostname, including a site's own "subdomain," is already an explicit `domains` table row (same as any custom domain). A Railway-assigned domain is just another row, added by `db:bootstrap-production` or the existing admin Domains UI — see docs/RAILWAY.md, "Domains."
- **The reverse-proxy/cookie/CSP surface was audited, not changed.** Session cookies (`secure` gated on `NODE_ENV=production`, `sameSite: "lax"`, `httpOnly`), Host-header trust (never `X-Forwarded-Host`), and CSP were all already correct for a TLS-terminating reverse proxy in front of the app — Railway's own edge fits the exact model docs/DEPLOYMENT.md's "Domains & TLS" already assumed. See docs/RAILWAY.md, "Admin behind Railway's reverse proxy."
- **Explicit position: no CD pipeline yet, deliberately.** A first deployment with a manual bootstrap step and a deliberately-manual migration step (see docs/RAILWAY.md, "Where do db:migrate/db:setup-roles actually run?") is not a good candidate for auto-deploy-on-push — that would either silently skip the migration step or turn a required human gate into automation wearing CD's clothing. A manual, reproducible `railway up` against a commit CI already turned green on is the honest posture for this release.
...
- **No self-service signup, password reset, or email verification.** A user is provisioned by an existing OWNER/ADMIN, the seed script, or (v1.0.2, production only) `pnpm db:bootstrap-production` for the very first owner of a new deployment — see docs/RAILWAY.md. None of that is self-service by an end user; there is still no "forgot password" flow. See [docs/AUTHENTICATION.md](AUTHENTICATION.md).
...
- **No MediaAsset deletion UI.** v0.9.1 adds the safety check a delete action would need (`isMediaAssetSafeToDelete` — checks current draft pages, branding, and every historical Revision, published-and-current or not) but still exposes no UI or Server Action to actually delete a MediaAsset. See [ADR 0022](adr/0022-media-ingestion-asset-delivery.md), Decision 14, and its v0.9.1 addendum.
- **Orphan reconciliation is detection-only, not automated cleanup.** v0.9.1 adds `findStorageOrphans`/`findDbOrphans` (`packages/media/src/reconciliation/orphan-scan.ts`) as callable, non-destructive primitives — nothing acts on their findings automatically, deliberately (prefer a human/deliberate step over auto-deleting anything that "looks" orphaned). See [docs/MEDIA.md](MEDIA.md).
- **`S3ObjectStorage`'s integration test runs against `s3rver`, not a real MinIO/AWS S3/R2 endpoint.** v0.9.1 closed the "zero test coverage" gap (MinIO/Docker is blocked by this environment's own egress policy; `s3rver`, a real S3-REST-API Node.js server, is the documented substitute) — but this still isn't a substitute for smoke-testing against your actual production bucket/provider before a first deploy. Still true after v1.0.1, which root-caused and fixed the separate `smoke:storage` script anomaly against `s3rver` (see "What v1.0.1 — Production Validation & Operational Hardening adds" above) but likewise had no real AWS S3/R2/MinIO credentials available to test against. See [docs/MEDIA.md](MEDIA.md).
...
- ~~`pnpm db:seed`/`pnpm db:publish-seed` have no `NODE_ENV` gate.~~ **Resolved in v1.0.1** — see "What v1.0.1 — Production Validation & Operational Hardening adds" above and docs/DEPLOYMENT.md, "Migrations."
...
- **No CD pipeline; real cloud deployment now documented and tooled, not yet exercised against live infrastructure.** v1.0.1 confirmed the three Docker images actually build and run (real GitHub Actions job evidence, real runtime smoke-testing in CI). v1.0.2 goes one step further: a concrete deployment target (Railway + Cloudflare R2) is fully documented (docs/RAILWAY.md) with real, working tooling — `db:verify-roles`, `db:bootstrap-production`, a real remote-browser smoke test — but no Railway or Cloudflare credentials exist in this development environment, so none of it has been run against a real managed Postgres, a real R2 bucket, or an actual live URL. Still deliberately no CD pipeline (see docs/RAILWAY.md, "CI/CD"). See the v1.0.2 final report's LIMITES / ACTIONS MANUELLES RESTANTES section for the complete, itemized handoff.

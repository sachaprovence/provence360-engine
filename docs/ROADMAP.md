...
- **No CD pipeline, no real cloud deployment exercised.** v1.0.1 confirmed the three Docker images actually build (real GitHub Actions job evidence, not just the workflow file) and added real container-runtime smoke-testing (start, health checks, non-root, clean SIGTERM) to CI — resolving the "not the final container layer itself" gap this bullet used to describe. What's still true, unchanged: no real cloud target and no deployment credentials exist for this repository, so nothing here has ever been exercised against a real managed Postgres, a real AWS S3/R2/MinIO bucket, or an actual production deployment. See the v1.0.1 final report's LIMITATIONS section for the complete, itemized list.
